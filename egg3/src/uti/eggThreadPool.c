#include "eggThreadPool.h"
void *tpool_thread(void *);

void eggThreadPool_init(tpool_t *tpoolp,int num_worker_threads,int max_queue_size,int do_not_block_when_full)
{
	int i, rtn;
	tpool_t tpool;
	...
	if ((rtn = pthread_mutex_init(&(tpool->queue_lock), NULL)) != 0)
		fprintf(stderr, "pthread_mutex_init %s", strerror(rtn)),
		    exit(1);
	if ((rtn =
	     pthread_cond_init(&(tpool->queue_not_empty), NULL)) != 0)
...
int eggThreadPool_add_work(tpool_t tpool, void (*routine) (void *), void *arg)
{
	int rtn;
	tpool_work_t *workp;
	...
	if ((tpool->cur_queue_size == tpool->max_queue_size) &&
		...
		if ((rtn =
		     pthread_mutex_unlock(&(tpool->queue_lock))) != 0)
	...
	if ((workp =
	     (tpool_work_t *) malloc(sizeof(tpool_work_t))) == NULL)
		perror("malloc"), exit(1);
	...
	if (tpool->cur_queue_size == 0) {
		...
		if ((rtn =
		    ...
		    0)
			fprintf(stderr, "pthread_cond_signal %d", rtn),
			    exit(1);;
	} else {
...
int eggThreadPool_destroy(tpool_t tpool, int finish)
{
	int i, rtn;
...
void *tpool_thread(void *arg)
{
	...
	for (;;) {
		...
		while ((tpool->cur_queue_size == 0) && (!tpool->shutdown)) {
			...
			printf("worker %x: I'm sleeping again\n",
			       pthread_self());
		...
		printf("worker %x: dequeing item %d\n", pthread_self(),
		       my_workp->next);
...
{
    HEGGTHREADTASKDETAILS lp_task_details = (HEGGTHREADTASKDETAILS) malloc (sizeof(EGGTHREADTASKDETAILS));
    lp_task_details->cnt = detailCnt;
    lp_task_details->details = (void**)malloc(sizeof(void*) * detailCnt);
    va_list ptr;
    va_start(ptr,detailCnt);

    
    index_t i = 0;
    while(i < detailCnt)
    {
        lp_task_details->details[i] = va_arg(ptr,void*);
        i++;
    }
    
    va_end(ptr);
    
    return lp_task_details;
}

EBOOL eggThreadPool_taskdetails_destroy(HEGGTHREADTASKDETAILS hThreadTaskDetails)
{
    free(hThreadTaskDetails->details);
    free(hThreadTaskDetails);
}
