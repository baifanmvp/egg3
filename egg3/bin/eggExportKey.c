        ...
        while (n_iter_off)
        {
            ...
            while(n_index_iter != EGGINDEXNODEVIEW_RDCNT(lp_node_view))
            {
                HEGGINDEXRECORD pRecord = EGGINDEXNODEVIEW_RECORD_INDEX(lp_node_view, n_index_iter);
                {
                    ...
                    printf("key : [%d] keylen : %d vallen : %d n_nodelist_off : %llu\n", *(int*)EGGINDEXRECORD_KEY(pRecord),EGGINDEXRECORD_KSIZE(pRecord), EGGINDEXRECORD_VSIZE(pRecord), n_nodelist_off);
                }
