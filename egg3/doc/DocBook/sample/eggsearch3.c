...
int main()
{
    ...
    HEGGQUERY hq1 = eggQuery_new_string("content", "is bad", strlen("is bad"), ANALYZER_CWSLEX);
    
    int time = 20120104;
    HEGGQUERY hq2 = eggQuery_new_int32("time", time);

    HEGGQUERY hq3 =  eggQuery_new_string("url", "www.god.com", strlen("www.god.com"), ANALYZER_CWSLEX);

    hq1 = eggQuery_and(hq1, hq2);

    hq1 = eggQuery_and(hq1, hq3);
    ...
    if (ret == EGG_TRUE)
    {
        ...
        while (i != cnt)
        {
            ...
            HEGGFIELD lp_field3 = eggDocument_get_field(lp_eggDocument, "url");
            unsigned len3 = 0;
            char *val3 = eggField_get_value(lp_field3, &len3);

            
            printf("id : [%llu], time : [%d] content : [%s], url : [%s] weight : [%d]\n", EGGDID_DOCID(&lp_score_doc[i].idDoc), *((int*)val1), val2, val3, eggDocument_get_weight(lp_eggDocument));
