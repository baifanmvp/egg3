...
typedef struct eggMemServerConfig_ eggMemServerConfig;
typedef struct eggMemServerConfig_ *HEGGMEMSERVERCONFIG;
struct eggMemServerConfig_ {
    char *socketfile;
    unsigned short port;
    char *ip;
};
HEGGMEMSERVERCONFIG eggMemServerConfig_new(char *configfile);
EBOOL eggMemServerConfig_delete(HEGGMEMSERVERCONFIG hEggMemServerConfig);
static int config_forceOverride = 0;
HEGGMEMSERVERCONFIG g_eggMemServerConfig = EGG_NULL;
...
HEGGMEMSERVER eggMemServer_new(HEGGMEMSERVERCONFIG p_eggConfig)
{
    ...
    if(p_eggConfig->socketfile)
    {
        ...
        long flag;
        flag = fcntl(unixfd, F_GETFL);
        flag |= O_NONBLOCK;
        if (fcntl(unixfd, F_SETFL, flag) < 0)
        {
    ...
    if(p_eggConfig->ip)
    {
        ...
        long flag;
        flag = fcntl(inetfd, F_GETFL);
        flag |= O_NONBLOCK;
        if (fcntl(inetfd, F_SETFL, flag) < 0)
        {
            perror("fcntl(F_SETFL) NONBLOCK");
            exit(EXIT_FAILURE);
        }
    ...
    default:
        ...
        for(idx = 0; idx < EGGMEMSERVER_FDNUM; idx++)
        {
            if(hServer->sockfd[idx] > 0 && FD_ISSET(hServer->sockfd[idx], &readfds))
            {
                ...
                long flag;
                flag = fcntl(requestfd, F_GETFL);
                flag |= O_NONBLOCK;
                if (fcntl(requestfd, F_SETFL, flag) < 0)
                {
...
EBOOL eggMemEggHandle_init(HEGGMEMEGGHANDLE hEggMemEggHandle, char *eggPath, char *analyzerName)
{
    ...
    if (eggPath[n-1] != '/')
    {
        eggPath_extr = malloc(n + 100);
        assert(eggPath_extr);
        sprintf(eggPath_extr, "file:///%%%%%%%s/", eggPath);
    }
    else
    {
        eggPath_extr = malloc(n + 100);
    ...
    hEggMemEggHandle->hEggHandle = (HEGGHANDLE)eggPath_open(hEggMemEggHandle->eggPath);
...
HEGGMEMSERVERCONFIG readcommandline(HEGGMEMSERVERCONFIG hEggMemServerConfig,
                                        int *argcp,
                                        char *argv[])
{
    if (!hEggMemServerConfig)
    {
        hEggMemServerConfig = eggMemServerConfig_new(NULL);
    }
    ...
    for (j = 1, i = 1; i < argc; )
    {
        if (strcmp(argv[i], "--unixsock") == 0)
        {
            ...
            free(hEggMemServerConfig->socketfile);
            hEggMemServerConfig->socketfile = strdup(argv[i+1]);
            assert(hEggMemServerConfig->socketfile);
        ...
        else if (strcmp(argv[i], "--ip") == 0)
        {
            ...
            free(hEggMemServerConfig->ip);
            hEggMemServerConfig->ip = strdup(argv[i+1]);
            assert(hEggMemServerConfig->ip);
        ...
        else if (strcmp(argv[i], "--port") == 0)
        {
            ...
            hEggMemServerConfig->port = strtol(argv[i+1], NULL, 10);
...
int main(int argc ,char* argv[])
{
    ...
    g_packPool = eggRWSPackBuf_new(eggPath);
    if (!g_packPool)
    {
        exit(-2);
    }
    ...
    eggRWSPackBuf_delete(g_packPool);
    ...
    free(eggPath);
    eggMemServerConfig_delete(g_eggMemServerConfig);
...
PRIVATE HEGGNETPACKAGE eggMemServer_processing_optimize(HEGGNETPACKAGE hNetPackage)
{
    ...
    ret = eggRWSPackBuf_add(g_packPool, hNetPackage);
...
HEGGMEMSERVERCONFIG eggMemServerConfig_new(char *configfile)
{
    HEGGMEMSERVERCONFIG hEggMemServerConfig;
    hEggMemServerConfig = (HEGGMEMSERVERCONFIG)
        malloc(sizeof(eggMemServerConfig));
    assert(hEggMemServerConfig);
    hEggMemServerConfig->socketfile = NULL;
    hEggMemServerConfig->port = 0;
    hEggMemServerConfig->ip = NULL;
    return hEggMemServerConfig;
